...
for(rep in 1:10){
  ...
  try_fail_catch(
    grp <- obj$lapply(
      ...
      name = paste0("chaibankmu_linked_0_nmf_TRUE_rep_", rep), overwrite = TRUE)
  )
...
grps <- lapply(obj$task_bundle_list(), function(x){obj$task_bundle_get(x)})[1:10]
...
dir.create(here::here("analysis/data-derived/sims_nmf"))

# first save the parms used to generate (X)
saveRDS(grps[[1]]$X, here::here("analysis/data-derived/sims_nmf/X.rds"))
X <- grps[[1]]$X
...
for(i in 94:1250) {
  ...
  if(any(unlist(lapply(r_i, object.size))==0)){
    ...
    saveRDS(r_i, here::here("analysis/data-derived/sims_nmf", fn_i))
    gc()
