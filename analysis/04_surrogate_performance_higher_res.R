...
pred_gg
save_figs("model_prediction_performance_higher_res", pred_gg, 14, 4, font_family = "Helvetica")
...
mod_perf_gg <- mod_perf %>% pivot_longer(RMSE:`1-R2`) %>%
  ...
  ylab("Value")
mod_perf_gg
save_figs("model_prediction_summary_performance_higher_res", mod_perf_gg, 6, 4, font_family = "Helvetica")
...
parm_grid$s_a_5 <- res_mod$get_model_predict_f()$brnn(res_mod$get_models()$s_a_5$brnn, parm_grid, s_name)
parm_grid$s_a_5 <- res_mod$predict_s(parm_grid, s_name)
...
save_figs("emulator_relation_higher_res", relation_gg, 6, 4, font_family = "Helvetica")
