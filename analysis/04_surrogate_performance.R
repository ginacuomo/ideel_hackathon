...
s <- "s_a_5"
dat <- test %>% select(-starts_with("s")) %>% as.matrix
...
pred_df$Model <- factor(pred_df$Model, levels = c("BRNN","MONMLP","XGB","Weighted Ensemble"))
...
mod_perf <- pred_df %>% group_by(Model) %>%
  ...
  mutate(Model =
           factor(Model, c("BRNN", "MONMLP", "XGB", "Weighted Ensemble"))
  )
...
aq_res <- 0.0
lu_res <- 0.0
ft <- c(0.06,0.12,0.2,0.3,0.4,0.6,0.8)
micro210 <- seq(0.02, 0.9, 0.01)
s_name <- "s_a_5"
parm_grid <- expand.grid(al, asaq,dhappq, art_res, ppq_res, aq_res, lu_res, ft, micro210)
colnames(parm_grid) <- c("al", "asaq","dhappq", "art_res", "ppq_res", "aq_res", "lu_res", "ft", "micro210")
...
preddf %>% ggplot(
  ...
  geom_smooth(aes(y = s_a_5_min), se = FALSE, lwd = 0.5, linetype = "dashed") +
  geom_smooth(aes(y = s_a_5_max), se = FALSE, lwd = 0.5, linetype = "dashed")  +
  theme_bw()
...
# Comparaitive model ensemble plot comparisons

huh <- res_mod$get_model_predict_f()$monmlp(res_mod$get_models()$s_a_5$monmlp, parm_grid, "s_a_5")
huh <- res_mod$get_model_predict_f()$xgb(res_mod$get_models()$s_a_5$xgb, parm_grid, "s_a_5")
huh <- res_mod$get_model_predict_f()$brnn(res_mod$get_models()$s_a_5$brnn, parm_grid, "s_a_5")
huh <- res_mod$get_model_predict_f()$brnn(a5_brnn, parm_grid, "s_a_5")
parm_grid %>% mutate(s_a_5 = huh) %>% ggplot(
  aes(micro210, s_a_5, color = as.factor(asaq))
) +
  geom_line()
...
lapply(paste0("s_a_",1:6), function(x){
  (mean(abs(test[[x]] - res_mod$predict_s(test,x))))
})

for(n in names(xgb_mod_list)) {
  message(mean(abs(test[[n]] - predict(xgb_mod_list[[n]], test %>% select(-starts_with("s")) %>% as.matrix))))
}

for(n in names(brnn_mod_list)) {
  message(mean(abs(test[[n]] - predict(brnn_mod_list[[n]], test %>% select(-starts_with("s")) %>% as.matrix))))
}
