...
# get scenario data
world_f <- function(scenario, bounds = FALSE, t = "t_s_a_5"){
  ...
  if(bounds){
    world <- world %>%
      filter(!is.na(!!sym(t))) %>%
      mutate(!!sym(t) := replace(!!sym(t), !!sym(t)>=40, NA)) %>%
      mutate(!!sym(t) := replace(!!sym(t), !!sym(t)<0, NA))
  }
  return(world)
}

# create map
create_map_t <- function(world, world_map_0, limits = FALSE, t = "t_s_a_5", s = "s_a_5") {

  gg_map <- world %>%
    #filter(micro20 > 0.0001) %>%
    ggplot() +
    #geom_sf(aes(fill = cut(ft, seq(0,1,0.2))), color = NA, show.legend = TRUE) +
    geom_sf(aes_string(fill = t), color = "grey", show.legend = TRUE, lwd = 0.1) +
    scale_fill_viridis_c(name = "Years", breaks = c(0,10,20,30,40), limits = c(0,40), option = "C", direction = -1, end = 0.7)


  # if(any((gg_map$data$micro210 < 0.0005))) {
  #
  #   gg_map <- gg_map +
  ...
  #       data = . %>% filter(micro210 < 0.0005), inherit.aes = FALSE) +
  ...
  if(any(is.na(gg_map$data[[t]]))) {
    ...
    gg_map <- gg_map +
      ggnewscale::new_scale_fill() +
      geom_sf(aes(fill = "blue"), color = NA, show.legend = TRUE, data = . %>% filter(is.na(!!sym(t)))) +
...
create_map_risk <- function(scenario_maps, scenario, isos, limits = FALSE, s = "s_a_5")  {
  ...
  gg_map_risk <- world %>%
    ggplot() +
    geom_sf(aes_string(fill = s), color = "grey", show.legend = TRUE, lwd = 0.1) +
    scale_fill_viridis_c(name = "Resistance Risk", option = "C", direction = 1, values = c(0,0.2, 1))

  # if(any((gg_map_risk$data$micro210 < 0.0005))) {
  #
...
for(i in seq_along(scenario_maps$map_data)) {
  compl[[i]] <- left_join(scenario_maps$map, scenario_maps$map_data[[i]]) %>% sf::st_drop_geometry() %>%
    select(iso, name_0, name_1, t, tmin, tmax, s, smin, smax, micro210, ft) %>%
